...
void nativeGetVideoFormat(int id, int& width, int& height, float& totalTime) {
	...
	width = (videoCtx->avhandler->getVideoInfo()).width;
	height = (videoCtx->avhandler->getVideoInfo()).height;
	totalTime = (float)((videoCtx->avhandler->getVideoInfo()).totalTime);
}
...
void nativeGetAudioFormat(int id, int& channel, int& frequency, float& totalTime) {
	...
	channel = (videoCtx->avhandler->getAudioInfo()).channels;
	frequency = (videoCtx->avhandler->getAudioInfo()).sampleRate;
	totalTime = (float)((videoCtx->avhandler->getAudioInfo()).totalTime);
}
...
void nativeLoadThumbnail(int id, float time, void* texY, void* texU, void* texV) {
	...
	int width = (int) (ceil((float) (avhandler->getVideoInfo()).width / ITextureObject::CPU_ALIGMENT) * ITextureObject::CPU_ALIGMENT);
	int height = (avhandler->getVideoInfo()).height;

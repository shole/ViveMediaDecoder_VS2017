...
bool AVHandler::isVideoBufferEmpty() {
	...
	return mIDecoder->getVideoInfo().isEnabled && mIDecoder->getVideoInfo().bufferState == EMPTY;
}
...
bool AVHandler::isVideoBufferFull() {
	...
	return mIDecoder->getVideoInfo().isEnabled && mIDecoder->getVideoInfo().bufferState == FULL;
